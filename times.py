...
def pause(now_time):
    ...
    start_pause = datetime.strptime(
        aux_ini, '%Y/%m/%d %H:%M').astimezone(local_timezone)
    end_pause = datetime.strptime(
        aux_fim, '%Y/%m/%d %H:%M').astimezone(local_timezone)
...
def time_to_send():
    delivery_time_ahead = datetime.now().astimezone(local_timezone) + timedelta(minutes=55)
